...
export const POST = async (req: any) => {
  try {
    const { userRecordId, sugarLevel } = await req.json();
    ...
    const predictionResponse = await fetch(
      "https://flutterdevelopers.tech/python/predict/dietplan",
      {
        method: "POST",
        headers: {
    ...
    return new Response(
      JSON.stringify({
        message: "Prediction received and processed successfully",
